...
| B23 | 🟠 | **Změna klávesy v nastavení nefunguje + nativní „Chce zapnout diktování?" popup se znovu objevuje na F5** (uživatel, 16. 7., zpráva 2) — nejpravděpodobnější příčina: **rebuild appky (nový ad-hoc podpis, viz R3) resetoval Accessibility/Input Monitoring granty**, takže `CGEventTapCreate` v `hotkey.py:_run()` selhal (vrací `None`) → celé vlákno tapu tiše skončilo (`return` na `hotkey.py:178`) → **žádný tap = F5 nikdy nedojde k `_callback` → nic ho nepotlačí → macOS ho zpracuje nativně** (proto se objevil systémový dictation popup, přestože je nativní diktování v Nastavení vypnuté — to jen brání spuštění diktování, ne zobrazení promptu). Zachytávání nové klávesy (`start_capture`) běží nad stejným mrtvým tapem → capture se nikdy nespustí → „Změnit" v UI vypadá jako by nic nedělalo. Nešlo ověřit naživo (chybí přístup k uživatelově GUI/klávesnici), jde o silnou hypotézu z reauditu kódu, ne potvrzenou diagnózu | 🟡 částečně opraveno, čeká retest | Přidán `on_tap_failed` callback do `HotkeyListener` (`hotkey.py`) — při selhání `CGEventTapCreate` appka teď pošle viditelnou notifikaci „Klávesa nefunguje — chybí Input Monitoring/Accessibility" místo tichého selhání (dřív `raise RuntimeError` v daemon vlákně = neviditelné). **Toto ale neopravuje samotné TCC oprávnění** — jen ho zviditelní. Skutečná oprava vyžaduje, aby uživatel po tomto rebuildu ručně znovu povolil Accessibility + Input Monitoring appce v Nastavení systému → Soukromí a zabezpečení, a appku restartoval. |
...
| R3 | 🟠 | **TCC vs. rebuildy** — ad-hoc podpis nemá stabilní identitu → **každý rebuild = macOS to bere jako jinou appku** a resetuje Keychain trust I Accessibility/Input Monitoring granty najednou. Potvrzený dopad v praxi: B22 (opakované Keychain dialogy) a hypotéza k B23 (mrtvý event tap → nefunkční hotkey + návrat nativního F5 diktování). Dlouhodobě neřešitelné bez placeného Apple Developer ID certifikátu (99 USD/rok) — s ním má appka jednu stabilní identitu napříč všemi buildy a macOS si oprávnění pamatuje trvale | Krátkodobě: po každém rebuildu ručně znovu povolit obě oprávnění (workflow, ne fix). Dlouhodobě: Developer ID podpis + notarizace — teprve tehdy TCC granty přežijí rebuild | Spike D / F3, dlouhodobě F4+ |
